// ==UserScript==
// @name         KaHack! Enhanced
// @version      1.1.0
// @namespace    https://github.com/jokeri2222
// @description  Enhanced Kahoot hack with rainbow mode and UI animations
...
(function() {
    ...
    function createParticles(element, color) {
        ...
        for (let i = 0; i < particleCount; i++) {
            ...
            particle.style.backgroundColor = color || colors.accent;
            particle.style.borderRadius = '50%';
            particle.style.pointerEvents = 'none';
            particle.style.zIndex = '10000';
            
            // Random position around the element
            const x = container.left + (Math.random() * container.width);
            const y = container.top + (Math.random() * container.height);
    ...
    uiElement.addEventListener('mouseenter', function() {
        this.style.boxShadow = colors.hoverGlow + ', 0 4px 20px rgba(0,0,0,0.3)';
    ...
    title.textContent = 'KaHack! Enhanced';
    ...
    function createToggle(labelText, checked, onChange) {
        ...
        input.addEventListener('change', function() {
            ...
            createParticles(slider, this.checked ? colors.correct : colors.incorrect);
        });
    ...
    answeringSection.body.appendChild(createToggle('Auto Answer', autoAnswer, function(checked) {
        ...
        createParticles(this, checked ? colors.correct : colors.incorrect);
    }));
    ...
    var rainbowToggle = createToggle('Enable Rainbow', false, function(checked) {
        if (checked) {
            startRainbowEffect();
            createParticles(this, colors.rainbow[Math.floor(Math.random() * colors.rainbow.length)]);
        } else {
            stopRainbowEffect();
        }
    });
    ...
    rainbowSection.body.appendChild(rainbowToggle);
    ...
    // Quiz ID validation with particle effect
    function handleInputChange() {
        ...
        fetch('https://kahoot.it/rest/kahoots/' + quizID)
            ...
            .then(function(data) {
                ...
                createParticles(inputBox, colors.correct);
            })
            .catch(function() {
    ...
    minimizeButton.addEventListener('click', function() {
        ...
        createParticles(minimizeButton, colors.minimize);
    });
    ...
    rainbowSlider.addEventListener('input', function() {
        ...
        if (isAltRPressed || rainbowToggle.querySelector('input').checked) {
            startRainbowEffect();
        }
    });
