...
// [Rest of your existing UI creation code...]
var Version = '1.0.25'
...
document.addEventListener('mousedown', function(e) {
    if (!forceCorrectMode || e.button !== 0 || info.questionNum === -1 || !questions[info.questionNum]) return;
    ...
    if (!currentQuestion || !currentQuestion.answers || currentQuestion.answers.length === 0) return;
    
    // Find the correct answer button
    const correctIndex = currentQuestion.answers[0];
    ...
    // Prevent the wrong answer from registering
    e.stopImmediatePropagation();
    e.preventDefault();
    
    // Trigger the correct answer
    correctBtn.click();
}, true); // Use capture phase to intercept before Kahoot's handlers
// ====== END HANDLER ====== //
