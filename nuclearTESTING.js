...
showAnswersInput.addEventListener('change', function() {
    ...
    if (info.questionNum !== -1 && questions[info.questionNum]) {
        if (showAnswers) {
...
keybindsLabel.innerHTML = 'Keybinds:<br>ALT+W - Answer correctly<br>ALT+S - Toggle answers';
...
function resetAnswerColors() {
    const answerButtons = document.querySelectorAll('button[data-functional-selector^="answer-"]');
...
document.addEventListener('keydown', function(e) {
    // Check if ALT+W is pressed and we're on a question
    ...
    // Check if ALT+S is pressed to toggle show answers
    if (e.key.toLowerCase() === 's' && e.altKey && info.questionNum !== -1 && questions[info.questionNum]) {
        e.preventDefault();
        e.stopPropagation();
        
        showAnswers = !showAnswers;
        showAnswersInput.checked = showAnswers;
