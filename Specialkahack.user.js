...
rainbowSpeedLabel.style.color = 'white';
...
rainbowSpeedSlider.addEventListener('input', () => {
    ...
    if (isAltRPressed) {
        startRainbowEffect(); // Restart with new speed if active
    }
});
...
keybindsLabel.style.color = 'white';
...
function startRainbowEffect() {
    ...
    const answerButtons = document.querySelectorAll('button[data-functional-selector^="answer-"], button[data-functional-selector^="multi-select-button-"]');
    answerButtons.forEach(button => {
        button.style.setProperty('background-color', colors[Math.floor(Math.random() * colors.length)], 'important');
        button.style.setProperty('transition', 'background-color 0.3s', 'important');
    });
...
function highlightAnswers(question) {
    question.answers.forEach(function (answer) {
        setTimeout(function() {
            ...
            if (btn) btn.style.setProperty('background-color', 'rgb(0, 255, 0)', 'important');
        }, 0)
    })
    question.incorrectAnswers.forEach(function (answer) {
        setTimeout(function() {
            const btn = FindByAttributeValue("data-functional-selector", 'answer-'+answer, "button");
            if (btn) btn.style.setProperty('background-color', 'rgb(255, 0, 0)', 'important');
        }, 0)
    })
}
